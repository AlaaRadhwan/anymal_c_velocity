...
def main() -> None:
  ...
  cfg.scene.sensors = (cfg.scene.sensors or ()) + (diagnostic_sensor_cfg,)
  ...
  stats = {name: ContactStats() for name in primary_names}
  ...
  number_of_env_steps = math.ceil(args.seconds / env.step_dt)
  ...
  for _ in range(number_of_env_steps):
    ...
    if force_history is None:
      raise RuntimeError("Contact force history was not created.")

    if distance_history is None:
      raise RuntimeError("Contact distance history was not created.")
    ...
    for substep_index in range(number_of_substeps):
      ...
      for geom_index, geom_name in enumerate(primary_names):
        ...
        contact_active = force > FORCE_EPSILON_N or distance < -PENETRATION_EPSILON_M
          ...
          contact_events.append(
            {
              ...
              "min_distance_m": (geom_stats.current_event_min_distance),
            }
          )
  ...
  actual_duration = total_physics_samples * env.physics_dt
  ...
  for geom_name in primary_names:
    ...
    contact_percentage = 100.0 * geom_stats.contact_samples / total_physics_samples

    total_contact_time = geom_stats.contact_samples * env.physics_dt

    longest_event_time = geom_stats.longest_event_samples * env.physics_dt

    mean_force = (
      geom_stats.force_sum / geom_stats.contact_samples
  ...
  events_path = Path(f"{args.output_prefix}_events.csv")
